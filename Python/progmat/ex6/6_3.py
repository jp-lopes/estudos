...
modelo_scip.setObjective(quicksum(c[i] * x[i] for i in range(n)), sense="maximize")

# 4. Montar as Restrições (usamos addCons no SCIP)
for j in range(m):
    modelo_scip.addCons(quicksum(a[j][i] * x[i] for i in range(n)) <= b[j])
